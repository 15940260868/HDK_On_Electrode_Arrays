...
'''
	1. 加载数据
	2. 映射多项式 (数据的feture可能很少，导致偏差大，所以创造出一些feture结合)
	3. 梯度下降(需正则化防止过拟合)（目标函数y = theta0 + theta1*X1 + theta2*X2 + theta3*X1*X2 + theta4*X1^2 + theta5*X2^2 ）
	4. 预测
'''

def LogisticRegression():
	data = loadtxtAndcsv_data("../documents/data2.txt", ",", np.float64)
	X = data[:, 0:-1]
	y = data[:, -1]
	...
	#print("result: {}".format(result)) (6，)
	#print("X.shape: {}".format(X.shape)) # (118, 6)
...
def gradient(initial_theta, X, y, inital_lambda):
	...
	temp = np.dot(X, initial_theta)
	#print("temp.shap{}".format(temp.shape)) # (118,)
	h = sigmoid(np.dot(X, initial_theta))  # 计算h(z)
